...
func NewWsService(manager *Manager) repository.SocialWebSocket {
	return &WsService{
		manager: manager,
		mu:      sync.RWMutex{},
	}
...
func (s *WsService) RegisterClient(userID int64) error {
	s.mu.Lock()
	defer s.mu.Unlock()
	...
	klog.Infof("用户 %d WebSocket连接已建立并注册", userID)
	return nil
}
...
func (s *WsService) JoinChatRoom(userID, roomID int64) error {
	...
	s.manager.SendToRoom(roomID, joinMsg)

	klog.Infof("用户 %d 加入聊天室 %d", userID, roomID)
	return nil
}
...
func (s *WsService) LeaveChatRoom(userID, roomID int64) error {
	...
	s.manager.SendToRoom(roomID, leaveMsg)

	klog.Infof("用户 %d 离开聊天室 %d", userID, roomID)
	return nil
}
...
func (s *WsService) SendChatMessage(userID, roomID int64, content string, msgType int8) error {
	...
	s.manager.SendToRoom(roomID, msg)
	klog.Infof("用户 %d 在聊天室 %d 发送消息: %s", userID, roomID, content)
	return nil
}
...
func (s *WsService) SendPrivateMessage(fromID, toID int64, content string) error {
	...
	klog.Infof("用户 %d 向用户 %d 发送私信: %s", fromID, toID, content)
	return nil
}
